...
{    
    ...
    symbol_t *new_p =(symbol_t *)malloc(sizeof(symbol_t));
    new_p->name = name;
    new_p->addr = vmaddr;
    new_p->next = symtab->next;
...
parse_t parse_reg(char **ptr, regid_t *regid)
{
    ...
    /* find register */
    tmp_reg = find_register(cur);
    if (tmp_reg == -1) return PARSE_ERR;
    cur += 4;
...
parse_t parse_imm(char **ptr, char **name, long *value)
{
    ...
    *ptr = cur;
    if(flag == 1) { *value = tmp_val; return PARSE_DIGIT;}
    if(flag == 2) { *name = tmp_name; return PARSE_SYMBOL;}
...
parse_t parse_label(char **ptr, char **name)
{
    ...
    while(cur[len] != ':')
    {
	if(IS_LETTER(cur+len) || IS_DIGIT(cur+len)) len++;
...
cont:
	...
	ret = parse_reg(&cur, &regidA);
        ...
        /* set y86bin codes */
	y86bin->codes[1] = HPACK(regidA, 0xF);
        goto cont;
      }
	...
	y86bin->codes[1] = HPACK(0xF, regidB);
