...
stat_t nexti(y86sim_t *sim)
{
	...
	long_t temp = get_reg_val(sim->r,regA);
	if (cond_doit(sim->cc,ifun)){
	    set_reg_val(sim->r,regB,temp);
	}
	sim->pc = next_pc;
	...
	long_t tmp_1 = get_reg_val(sim->r,regA);
	long_t tmp_2 = get_reg_val(sim->r,regB);
	tmp_2 += imm;
	if(!set_long_val(sim->m,tmp_2,tmp_1))
	{
	    err_print("PC = 0x%x, Invalid data address 0x%x", sim->pc, tmp_2)
	    return STAT_ADR;
	}
	sim->pc = next_pc;
	break;
	...
	if(ifun != 0 || !NORM_REG(regA) || !NORM_REG(regB)) return STAT_INS;
	long_t tmp = get_reg_val(sim->r,regB);
	tmp += imm;
	long_t tmp3 = 0;
	if(!get_long_val(sim->m,tmp,&tmp3))
	{
	    err_print("PC = 0x%x, Invalid data address 0x%x", sim->pc, tmp);
       	    return STAT_ADR;
	}	
	set_reg_val(sim->r,regA,tmp3);
	sim->pc = next_pc;
	break;
      }
	...
	long_t tmp_1 = get_reg_val(sim->r,regA);
	long_t tmp_2 = get_reg_val(sim->r,regB);
	long_t ans = compute_alu(ifun, tmp_1, tmp_2);
	sim->cc = compute_cc(ifun, tmp_1, tmp_2, ans);
	...
	if(!(ifun >= C_YES && ifun <= C_G)) return STAT_INS;
	    ...
	    long_t tmp_1 = get_reg_val(sim->r, REG_ESP)-4;
	    set_reg_val(sim->r, REG_ESP, tmp_1);
	    if(!set_long_val(sim->m, tmp_1, next_pc))
	    {
		err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, tmp_1);
		return STAT_ADR;
	    }
	    sim->pc = imm;
	...
	long_t tmp = get_reg_val(sim->r, REG_ESP);
	if(!get_long_val(sim->m, tmp, &sim->pc))
	{
	    err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, tmp);
       	    return STAT_ADR;
	}
	tmp += 4;
	set_reg_val(sim->r, REG_ESP, tmp);
	break;
      }

      case I_PUSHL:{ /* A:0 regA:F */
	if(ifun != 0 || !NORM_REG(regA) || !NONE_REG(regB)) return STAT_INS;
	long_t tmp1 = get_reg_val(sim->r, regA);
	long_t tmp2 = get_reg_val(sim->r, REG_ESP)-4;
	set_reg_val(sim->r, REG_ESP, tmp2);
	if(!set_long_val(sim->m, tmp2, tmp1)){
	...
	long_t tmp2 = 0;
	long_t tmp3 = tmp+4;
	if(!get_long_val(sim->m, tmp, &tmp2))
	{
	    err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, tmp);
	    return STAT_ADR;
	}
	set_reg_val(sim->r, REG_ESP, tmp3);
	set_reg_val(sim->r, regA, tmp2);
	sim->pc = next_pc;
        break;
      }
