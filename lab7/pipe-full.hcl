...
## 主要思路：
## 1.添加iaddl和leave指令
## 2.增添mrmovl+rmmovl情况下的组合逻辑（旁路转发）
## 3.为ys的jle的情况做一个特别判断，是插入的bubble减少到1或0.
...
int f_pc = [
	...
	(M_icode == IJXX && !M_Cnd) && 
		!(M_ifun == 1 && W_icode == IOPL && E_icode == IIADDL): M_valA;
...
int f_predPC = [
	f_icode in { IJXX, ICALL } &&
		 !(f_ifun == 1 && D_icode == IOPL && D_ifun == 0): f_valC;
	1 : f_valP;
];
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd)  && !(E_ifun == 1 && M_icode == IOPL && D_icode == IIADDL) ||
	# Stalling at fetch while ret passes through pipeline
